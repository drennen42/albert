...
var states = [' ', 'AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA', 'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY', 'AB', 'BC', 'MB', 'NB', 'NL', 'NT', 'NS', 'NU', 'ON', 'PE', 'QC', 'SK', 'YT'].sort();
...
$('select#address-state-input, select#billing-state-input').trigger("chosen:updated");
...
function billingSameCheckboxHandler() {
	if ($('#billing-address-same-input')[0].checked) {
		...
		$('#billing-state-input').val($('#address-state-input').val()).trigger('chosen:updated');
		...
		$('#billing-state-input').val('').trigger('chosen:updated');
